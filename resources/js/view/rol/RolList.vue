                    ...
                    <div class="card-header border-0">
                        <div class="row">
                            <div class="col-12 col-sm-3">
                                <h3 class="title-form">Lista de Rol</h3>
                            </div> 
                        ...
                        <table class="table align-items-center table-flush table-hover">
                            ...
                            <tbody>
                                <tr class="list" v-for="( data, index ) in dataList" :key=" index" >
                                    ...
                                    <td class="text-muted">
                                        <!-- <div class="form-row mx-3" v-for="(permission, index) in data.permissions_list" :key="index+'permissions'" > -->
                                        <div class="form-row mx-3" >
                                            <!-- <label class="form-control-label col-12">{{ permission.name }}</label>  -->

                                            <ul class="col-12" v-for="( moduleUser, index ) in data.module" :key="index+'permissions'">
                                                <li class="">{{ moduleUser.name }}</li>
                                                <div>
                                                    <ul class="col-12" v-for="( permissions, index ) in moduleUser.permissions" :key="index+'permissions'">
                                                        <li class="">{{ permissions.name }}</li>
                                                    </ul>
                                                </div>
                                            </ul>
                                        </div>
    ...
    methods: {
        ...
        async activeOrDesactiveItem( id, status ){
            ...
            try {
                ...
                const response = await axios.post( `rol-active-desactive`, body )
        ...
        async getApiList( searchName ){
            try {
                ...
                const response = await searchName ? await axios.post( 'rol-search', body  ) : await axios( 'rol' )
