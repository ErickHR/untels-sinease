<template>
    <div class="justify-content-center">
        <div class="row justify-content-center">
            <div class="col-lg-10 col-md-10 col-sm-10 col-10 ">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-lg-12">
                                <h3 class="mb-2 title-form d-inline-block">
                                    ESTADOS DE DOCUMENTOS
                                </h3>
                            </div>
                        </div> 
                        <div class="opciones-section mt-1 pb-2">
                            <small class="text-muted">
                                <strong>
                                    Descripción:
                                </strong> 
                                Estas son las estados por las que va a pasar los documentos
                            </small>
                        </div> 
                        <div class="origen-section ">
                            <div class="row pr-3">
                                <div class="col-12 col-sm-12 col-xl-12 ">
                                    <div class="form-row">
                                        <label class="form-control-label col-12">NOMBRE</label> 
                                        <span class="input-widht col-12">
                                            <input type="text" :placeholder="'Coloque el nombre para '+ nameButton.toLowerCase() " class="form-control text-primary col-12" v-model="dataForm.name"> 
                                            <span class="is-invalid-numero text-red"></span>
                                        </span>
                                    </div>
                                </div> 

                                <div class="col-12 col-sm-12 col-xl-12 ">
                                    <div class="form-row">
                                        <label class="form-control-label col-12" for="color">COLOR</label> 
                                        <span class="input-widht col-12">
                                            <input type="color" class="form-control form-control-color" id="color" v-model="dataForm.color"> 
                                            <span class="is-invalid-numero text-red"></span>
                                        </span>
                                    </div>
                                </div>

                                <div class="col-12 col-sm-12 col-xl-12 ">
                                    <div class="form-row">
                                        <label class="form-control-label col-12" for="color">NECESITA MENSAJES?</label> 
                                        <div class="col-md-3 col-12 mx-3">
                                            <div class="custom-control custom-radio mb-3">
                                                <input name="need-msg" class="custom-control-input" id="nedd-msg-false" v-model="dataForm.need_msg" value="0" type="radio">
                                                <label class="custom-control-label" for="nedd-msg-false">NO</label>
                                            </div>
                                        </div>
                                        <div class="col-md-3 col-12 mx-3">
                                            <div class="custom-control custom-radio mb-3">
                                                <input name="need-msg" class="custom-control-input" id="nedd-msg-true" v-model="dataForm.need_msg" value="1" type="radio">
                                                <label class="custom-control-label" for="nedd-msg-true">SÍ</label>
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                <div class="col-2 col-sm-2 col-xl-2">
                                    <br> 
                                    <button type="button" @click="apiForm"  class="btn" :class="[colorButton]">
                                        {{ nameButton }} <i class="fas" :class="[iconButton]"></i>
                                    </button>
                                </div>
                                <div class="col-2 col-sm-2 col-xl-2" v-if=" action != 'search' ">
                                    <br> 
                                    <button type="button" @click="cancelRegister"  class="btn btn-warning">
                                        Cancelar <i class="fas fa-close" ></i>
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
export default {
    
    data() {
        return {
            action : 'search',
            dataForm : {
                name : '',
                color : '#000000',
                need_msg : 0
            },
            idEdit : null
        }
    },

    methods: {

        cancelRegister(){

            this.resetForm()

        },
        
        async showEditItem(id){
            try {
                
                let response = await axios.get( `status-file/${id}` )

                if( response.status == 200 ) {

                    if( response.data.value ) {

                        Alerts.success( response.data.msg )

                        this.action = 'update'
                        this.idEdit = id
                        this.dataForm.name = response.data.data.name

                    } else {

                        Alerts.error( response.data.msg )

                    }

                } else {

                    Alerts.error( response.status + ': Error, no especificado.' )

                }

            } catch ( e ) {
                
                Alerts.error( 'Perdida de conexión ó Error de sistema.' )

            }

        },

        async apiForm(){

            if( this.action == 'search' ){
                this.$emit( 'searchItem', this.dataForm.name )
                return
            }

            try {
                
                let response = await this.axiosForm( 'status-file', '' )

                if( response.status == 200 ) {

                    if( response.data.value ) {
                        
                        this.resetForm()
                        await Alerts.success( response.data.msg )
                        this.$emit( 'updateList' )

                    } else {

                        Alerts.error( response.data.msg )

                    }

                } else {

                    Alerts.error( response.status + ': Error, no especificado.' )

                }

            } catch ( e ) {

                Alerts.error( 'Perdida de conexión ó Error de sistema.' )
                
            }

        },

        axiosForm( url ){

            if( this.action == 'search' ) {
                return axios.get( url )
            }

            if( this.action == 'update' ) {
                let body = { ...this.dataForm }
                return axios.put( `${url}/${this.idEdit}`, body )
            }

            if( this.action == 'add' ) {
                let body = { ...this.dataForm }
                return axios.post( url, body )
            }

        },

        changeActionOfRegister( action ){
            this.action = action
        },

        resetForm(){

            this.changeActionOfRegister( 'search' )
            this.dataForm.name = ''

        },

        capitalize(){
            var re = /(\b[a-z](?!\s))/g;
            var s = "fort collins, croton-on-hudson, harper's ferry, coeur d'alene, o'fallon"; 
            s = s.replace(re, function(x){return x.toUpperCase();});
            console.log(s);
        }

    },

    mounted() {
        
    },

    computed : {
        nameButton(){
            if( this.action == 'search' ) {
                return 'Buscar';
            }

            if( this.action == 'update' ) {
                return 'Editar';
            }

            if( this.action == 'add' ) {
                return 'Agregar';
            }
        },
        iconButton(){
            if( this.action == 'search' ) {
                return 'fa-search';
            }

            if( this.action == 'update' ) {
                return 'fa-pencil';
            }

            if( this.action == 'add' ) {
                return 'fa-plus';
            }
        },
        colorButton(){
            if( this.action == 'search' ) {
                return 'btn-primary';
            }

            if( this.action == 'update' ) {
                return 'btn-warning';
            }

            if( this.action == 'add' ) {
                return 'btn-success';
            }
        }
    }


}
</script>